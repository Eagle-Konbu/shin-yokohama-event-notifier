...
When modifying Terraform files (`terraform/` directory):

## Before Committing

1. **Auto-fix formatting and docs:**
   ```bash
   task tf-fix
   ```
...
3. **If checks fail:**
   - **Format issues:** Auto-fixed by `task tf-fmt`
   - **Validation errors:** Review error message and fix manually
   - **Lint issues:** Review TFLint output and fix based on recommendations
   - **Security warnings:** Review tfsec report
     - If false positive: Add exclusion to `terraform/.tfsec/config.yml`
     - If real issue: Fix the security vulnerability
   - **Documentation:** Auto-generated by `task tf-docs`

## CI Checks

The following checks run automatically on PRs:
- ✅ **Format check** (blocking) - Code must be formatted with `terraform fmt`
- ✅ **Validation** (blocking) - Configuration must be syntactically valid
- ✅ **Linting** (blocking) - Must pass TFLint AWS rules
- ⚠️ **Security scan** (warning only) - tfsec findings posted as PR comments
- ✅ **Documentation** (blocking) - README must be up to date
- ⚠️ **Version check** (warning only) - Terraform version constraint verified
