...
このドキュメントは、各施設のスクレイパー実装に関するGitHub Issueの内容を記載しています。

---

## Issue 1: 日産スタジアムのスクレイパー実装
...
#### 概要
横浜アリーナの公式サイトからイベント情報を取得するスクレイパーを実装します。

#### 対象URL
https://www.yokohama-arena.co.jp/event

#### 現状
- `internal/infrastructure/scraper/yokohama_arena.go` にスケルトン実装が存在
- `FetchEvents` メソッドは "not implemented" エラーを返す状態

#### 実装要件

1. **イベント情報の取得**
   - 横浜アリーナの公式イベントページからイベント情報をスクレイピング
   - 当日のイベント情報を取得
...
4. **テスト**
   - ユニットテストの実装（`yokohama_arena_test.go`）
   - モックを使用したテストケースの作成
...
#### 参考情報
- スクレイピング対象: https://www.yokohama-arena.co.jp/event
- 既存のスケルトン実装: `internal/infrastructure/scraper/yokohama_arena.go`
- インターフェース: `internal/domain/ports/event_fetcher.go`
- ドメインモデル: `internal/domain/event/event.go`

---

## Issue 3: KOSÉ新横浜スケートセンターのスクレイパー実装
...
このドキュメントの各 Issue セクションの内容を使用して、GitHub 上で Issue を作成してください。

1. GitHub リポジトリの Issues タブを開く
2. "New issue" をクリック
3. 上記の "Title" をタイトルフィールドにコピー
4. "Description" 以下の内容を本文にコピー
5. 適切なラベルを付与（例: `enhancement`, `scraper`, `good first issue` など）
6. "Submit new issue" をクリック

各 Issue は独立して実装可能なので、並行して作業を進めることも可能です。
